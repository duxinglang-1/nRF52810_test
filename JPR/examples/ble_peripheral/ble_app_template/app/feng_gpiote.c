...
void tp_interrupt_handler(void)
{
		...
		#if 1	
		tmpbuf[count_len++]=0xAB ;//头
		tmpbuf[count_len++]=0x00 ;//长度
		tmpbuf[count_len++]=0x0A ;//长度

		tmpbuf[count_len++] = 0xFF;//ID 校验开始 到校验和之前
		tmpbuf[count_len++] = 0xB1;	//ID

		tmpbuf[count_len++] = tp_temp[tp_len++] ;	//data  方向，上下，左右
		tmpbuf[count_len++] = tp_temp[tp_len++] ;	//data
		...
		for(i = 0;i<count_len;i++)//从tmpbuf[0]到tmpbuf[10]校验和
		{
			 tmpbuf[count_len+1]+=tmpbuf[i];
		}

		 tmpbuf[count_len++]=tmpbuf[count_len+1];//从tmpbuf[0]到tmpbuf[10]校验和	

		tmpbuf[count_len++] = 0x88; 	
